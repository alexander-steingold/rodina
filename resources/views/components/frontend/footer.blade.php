<!-- footer area start -->
<footer class="footer-area style-2">
    <div class="footer-top">

    </div>
    <div class="container">
        <div class="row">
            <div class="col-xl-5 col-lg-6">
                <div class="widget widget_about">
                    <x-frontend.logo/>
                    <div class="details mt-2">
                        <p>
                            Динамично развивающаяся компания, с 25-им опытом. Профессиональный и надежный поставщик, на
                            рынке услуг логистики, экспресс-доставки международных посылок и перевозок.
                        </p>
                        <ul class="social-media style-border">
                            <li>
                                <a href="https://api.whatsapp.com/send?phone=9720501234567&text=%D0%97%D0%B0%D0%B8%D0%BD%D1%82%D0%B5%D1%80%D0%B5%D1%81%D0%BE%D0%B2%D0%B0%D0%BD/%D0%B0%20%D0%B2%20%D0%B2%D0%B0%D1%88%D0%B5%D0%BC%20%D1%81%D0%B5%D1%80%D0%B2%D0%B8%D1%81%D0%B5"
                                   target="_blank"><i class="fab fa-whatsapp"></i></a></li>
                            <li><a href="#"><i class="fab fa-facebook-f"></i></a></li>
                        </ul>
                    </div>
                </div>
            </div>
            <div class="col-xl-1 col-lg-6">

            </div>
            <div class="col-xl-3 col-lg-6">
                <div class="widget widget_contact">
                    <h5 class="text-white">ГЛАВНЫЙ ОФИС</h5>
                    <div class="single-contact-inner">
                        <div class="icon">
                            <i class="fa fa-map-marker-alt"></i>
                        </div>
                        <div class="details">
                            <p class="mt-2">Беер Шева</p>
                        </div>
                    </div>
                    <div class="single-contact-inner">
                        <div class="icon">
                            <i class="fas fa-mobile-alt"></i>
                        </div>
                        <div class="details">
                            <p class="mt-2">+972 0501234567</p>
                        </div>
                    </div>
                    <div class="single-contact-inner">
                        <div class="icon">
                            <i class="far fa-clock"></i>
                        </div>
                        <div class="details">
                            <p class="mt-2">
                                Вс–Чт: 9.00-17.00
                            </p>
                        </div>
                    </div>
                </div>
            </div>
            <div class="col-xl-3 col-lg-6">
                <div class="widget widget_contact">
                    <h5 class="text-white">CALL-ЦЕНТР</h5>
                    <div class="single-contact-inner">
                        <div class="icon">
                            <i class="fa fa-map-marker-alt"></i>
                        </div>
                        <div class="details">
                            <p class="mt-2">Тель Авив</p>
                        </div>
                    </div>
                    <div class="single-contact-inner">
                        <div class="icon">
                            <i class="fas fa-mobile-alt"></i>
                        </div>
                        <div class="details">
                            <p class="mt-2">+972 0501234567</p>
                        </div>
                    </div>
                    <div class="single-contact-inner">
                        <div class="icon">
                            <i class="far fa-clock"></i>
                        </div>
                        <div class="details">
                            <p class="mt-2">
                                Вс–Чт: 8.00-18.00
                            </p>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</footer>
<!-- footer area end -->
